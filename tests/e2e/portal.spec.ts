...
test('team page uses the registered Projeto Big Data IESB roster and available photos', async ({ page }) => {
  ...
  const felipeCard = page.locator('.team-card').filter({ hasText: 'Felipe Melo Moreira Nunes' })
  await expect(felipeCard.getByRole('img', { name: 'Foto de Felipe Melo Moreira Nunes' })).toHaveAttribute('src', '/img/team/felipe-moreira-v2.jpg')
